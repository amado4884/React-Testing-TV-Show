...
test("Loading image", async () => {
  const { getByAltText } = render(<App />);
  await waitFor(() => {
    const image = getByAltText(/Stranger Things/i);
    expect(image.src).toBe("http://static.tvmaze.com/uploads/images/original_untouched/200/501942.jpg");
  });
});

test("Loading season 1", async () => {
  const { getByAltText } = render(<Episodes episodes={episodes} />);
  await waitFor(async () => {
    const ch1 = getByAltText(/Chapter One: The Vanishing of Will Byers/i);
    expect(ch1.src).toBe("http://static.tvmaze.com/uploads/images/medium_landscape/67/168918.jpg");
  });
});
